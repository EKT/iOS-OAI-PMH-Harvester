...
@synthesize oaiRecord, thumbnailData, pages;
...
- (NSString *)getPage:(int)page {
    if (!self.pages){
        ...
        NSString *basePath = [NSString stringWithFormat:@"%@/%@", APP_CACHE_FOLDER, [self getIdentifier]];
        ...
        NSString *bookPath = [NSString stringWithFormat:@"%@/%@/book.txt", APP_CACHE_FOLDER, [self getIdentifier]];
        if (![fileManager fileExistsAtPath:bookPath]){
            //Must download book.txt and read it!
            NSData *bookTxtData = [NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:URL_BOOK_TXT, [self getDigitalIdentifier]]]];
